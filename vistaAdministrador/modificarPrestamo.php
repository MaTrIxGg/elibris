        ...
        <nav class="navbar-user-top full-reset">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-xs-12 lead">
                        <ol class="breadcrumb">
                            <li><a href="<?= REGISTRAR_PRESTAMO['url'] ?>">Nuevo Préstamo / </li>
                            <li><a href="<?= CONSULTAR_PRESTAMO['url'] ?>">Listado de Préstamo</a></li>
                            <li class="active">Modificar de datos Préstamo</li>
                        </ol>
                    </div>
                </div>
            </div>
            <div class="container-fluid">
                <div class="container-flat-form">
                    <div class="title-flat-form title-flat-blue">Modificación de datos del Préstamo </div>
                    <form autocomplete="off" method="POST" action="<?= MODIFICAR_PRESTAMO['url'] . '?idprestamo=' . $prestamo->getIdprestamo() ?>">
                        <div class="row">
                        <div class="col-xs-offset-2 col-md-8 col-sm-offset-3">
                            ...
                            <div class="col-xs-12 col-sm-8 col-sm-offset-2">
                                <div class="group-material">
                                    ...
                                    <select name="pre_idejemplar"class="tooltips-general material-control" value="<?= $resultadoPrestamo->idejemplar ?>">
                             <?php
                                          if($resultadoPrestamo->idejemplar  == 0){
                                            ?> 
                                             <option value="-1">Seleccionar</option>
                                              <?php 
                                          }else{
                                        ?> 
                                           <option value="<?= $resultadoPrestamo->idejemplar ?>">><?= $resultadoPrestamo->codigounico ?></option>   
                                           <?php  
                                          }                                        
                                       ?>  
                                                                                                                                                       <?php 
                                            foreach ($listaejemplarPreM as $resultadoEjemplarpre) { 
                                              if($prestamo->getIdejemplar()==$resultadoEjemplarpre->idejemplar){
                                        ?>
                                    <option selected="" value="<?= $resultadoEjemplarpre->idejemplar ?>"><?= $resultadoEjemplarpre->codigounico ?></option>
                                        <?php        
                                              }else{
                                                
                                                ?>
                                    <option value="<?= $resultadoEjemplarpre->idejemplar ?>"><?= $resultadoEjemplarpre->codigounico ?></option>
                                    <?php
                                              }
                        ...
                        <div class="col-xs-12 col-sm-8 col-sm-offset-2"> 
                            <div class="group-material">                              
                                <span>Usuario</span>
                                 <select name="pre_idusuario"class="tooltips-general material-control" value="<?= $resultadoPrestamo->idusuario ?>">
                                  <?php
                                          if($resultadoPrestamo->idusuario  == 0){
                                            ?> 
                                             <option value="-1">Seleccionar</option>
                                              <?php 
                                          }else{
                                        ?> 
                                           <option value="<?= $resultadoPrestamo->idusuario ?>">><?= $resultadoPrestamo->numerodocumento ?></option>   
                                           <?php  
                                          }                                        
                                       ?>  
                                                                                                                                                       <?php 
                                            foreach ($listausuarioPreM as $resultadoUsuariopre) { 
                                              if($prestamo->getIdusuario()==$resultadoUsuariopre->idusuario){
                                        ?>
                                    <option selected="" value="<?= $resultadoUsuariopre->idusuario ?>"><?= $resultadoUsuariopre->numerodocumento ?></option>
                                        <?php        
                                              }else{
                                                
                                                ?>
                                    <option value="<?= $resultadoUsuariopre->idusuario ?>"><?= $resultadoUsuariopre->numerodocumento ?></option>
                                    <?php
                                              }
                        ...
                        <div class="col-xs-12 col-sm-8 col-sm-offset-2">
                            <div class="group-material">
                                <input type="date" name="pre_fechaprestamo" value="<?= $prestamo->getFechaprestamo() ?>" class="material-control tooltips-general" placeholder="Fecha del préstamo" required="" pattern="">
                        ...
                        <div class="col-xs-12 col-sm-8 col-sm-offset-2">
                            <div class="group-material">
                                <input type="date" name="pre_fechalimite" value="<?= $prestamo->getFechalimite() ?>" class="material-control tooltips-general" placeholder="Fecha límite del préstamo" required="" pattern="">
                                <span class="highlight"></span>
                                <span class="bar"></span>
                            ...
                            <div class="col-xs-offset-2 col-md-8 col-sm-offset-3">
                             ...
                             <div class="col-xs-12 col-sm-8 col-sm-offset-2">                                                                                  
                                <div class="group-material">
                                    <span>Estado</span>
                                     <select name="pre_idestado"class="tooltips-general material-control" value="<?= $resultadoPrestamo->idestado ?>">
                                         <?php
                                          if($resultadoPrestamo->idestado  == 0){
                                            ?> 
                                             <option value="-1">Seleccionar</option>
                                              <?php 
                                          }else{
                                        ?> 
                                           <option value="<?= $resultadoPrestamo->idestado ?>">><?= $resultadoPrestamo->nombreestado?></option>   
                                           <?php  
                                          }                                        
                                       ?>  
                                                                                                                                                       <?php 
                                            foreach ($listaestadoPreM as $resultadoEstadopre) { 
                                              if($prestamo->getIdestado()==$resultadoEstadopre->idestado){
                                        ?>
                                    <option selected="" value="<?= $resultadoEstadopre->idestado ?>"><?= $resultadoEstadopre->nombreestado ?></option>
                                        <?php        
                                              }else{
