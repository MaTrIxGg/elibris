...
WHERE P.fechaprestamo BETWEEN '20180301' AND '20180331'";
	$resultado = $mysqli->query($query);	
	$pdf = new PDF('P','mm','Letter');
	$pdf->AliasNbPages();
	$pdf->AddPage();	
        $pdf->SetFont('Arial','I',12);          
     $pdf->MultiCell(0, 5,utf8_decode('En este documento se puede evidenciar los préstamos realizados en el mes de Marzo, teniendo en cuenta los siguientes datos (número de documento del estudiante o docente, los nombres y apellidos del mismo, el código del ejemplar que se préstamo y la fecha en que se hizo el préstamo '), 0,'L',0);     
     $pdf->Ln();
     $pdf->SetFont('Arial','B',14);  
     $pdf->MultiCell(0, 5, 'MARZO', 0,'C',0);
     $pdf->Ln();        
